...
fn test_get_command_uses_default_shell_when_unspecified() -> anyhow::Result<()> {
    ...
    let args: ExecCommandArgs = parse_arguments("exec_command", json)?;
...
#[test]
fn exec_command_args_coerces_string_integers() {
    let args: ExecCommandArgs = serde_json::from_str(
        r#"{"cmd":"echo hi","yield_time_ms":"1500","timeout_ms":"120000","max_output_tokens":"2000"}"#,
    )
    .expect("string integer literals should parse");
    assert_eq!(args.yield_time_ms, 1500);
    assert_eq!(args.timeout_ms, Some(120000));
    assert_eq!(args.max_output_tokens, Some(2000));

    let args: ExecCommandArgs = serde_json::from_str(r#"{"cmd":"echo hi","timeout_ms":"0123"}"#)
        .expect("leading zeros should parse");
    assert_eq!(args.timeout_ms, Some(123));

    let args: ExecCommandArgs = serde_json::from_str(
        r#"{"cmd":"echo hi","yield_time_ms":250,"timeout_ms":30000,"max_output_tokens":1024}"#,
    )
    ...
    let args: ExecCommandArgs =
        serde_json::from_str(r#"{"cmd":"echo hi","timeout_ms":null,"max_output_tokens":null}"#)
            .expect("null should map to None");
    assert_eq!(args.timeout_ms, None);
    assert_eq!(args.max_output_tokens, None);

    let err = serde_json::from_str::<ExecCommandArgs>(r#"{"cmd":"echo hi","timeout_ms":"-5"}"#)
    ...
    for bad in [
        ...
        let err = serde_json::from_str::<ExecCommandArgs>(&format!(
            r#"{{"cmd":"echo hi","timeout_ms":"{bad}"}}"#
        ))
    ...
    for bad in ["1.5", "true", "[30000]", r#"{"ms":30000}"#] {
        assert!(
            serde_json::from_str::<ExecCommandArgs>(&format!(
                r#"{{"cmd":"echo hi","timeout_ms":{bad}}}"#
            ))
            .is_err(),
