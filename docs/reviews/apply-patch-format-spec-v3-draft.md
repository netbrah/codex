...
Status: SPEC — v3 (post round-3 review; resolution maps in §7). No code changes
until a fresh review round (round 4) returns zero Blocking and zero Major.
...
(qwen count as of the pinned snapshot instant in §1.2 — the session
directory is live; a fresh 2026-09-16 recount gives 439 calls, 6 PARSE ≈
1.4%.)
...
Method: every rollout under `~/.codex/sessions/2026-09/{13,14,15}/`;
...
build; OTHER = other rejections. **Snapshot (v3):** the session directory
is live (rollout lines are still being appended), so every count below is
pinned to **2026-09-15T23:30:11Z** — the instant the qwen cumulative call
count first reached 413 (83/162/86 rollout files for 09-13/09-14/09-15).
...
- Round-2 seat D strict recount (canonical for the glm row): glm 26/37 —
  reproduced exactly, event-by-event, by round-3 seats E and F; no glm
  data after 09-15 19:31 EDT, so the glm row is snapshot-stable.
...
- Freshness (v3): a fresh strict recount at the 2026-09-16T00:22Z snapshot
  gives qwen 439 calls with 6 PARSE (≈1.4%) — the ~1% rate holds; the glm
  row is unchanged.
...
- The FORMAT block is the **canonical** form the model must emit; both the
  ...
  prefixed line (streaming_parser.rs:307-347; probed on the unmodified
...
| C-N1 / D-m1 false `@@` note | Replaced with the verified statement: parser auto-creates the first chunk from a prefixed line, no `@@` required (streaming_parser.rs:307-347, probed, upstream-pinned at parser.rs:402; v3: range corrected); real deltas the other way (zero-chunk hunk rejected, second `@@` on empty chunk rejected, new `@@` after `*** End of File` accepted) | §3.1 notes |
...
| F-F3 | `invalid patch: ` Display prefix added to both §1.1 F4 quotes; parser-level vs harness-level strings distinguished | §1.1 F4 |
...
| E-N7 | Line ranges corrected to the current tree (auto-chunk streaming_parser.rs:307-347; `check_start_and_end_lines_strict` parser.rs:256-274, messages at :268/:271; responses.rs pattern cited by name + line); ranges re-drift under P3's own edits, so code cites symbols where possible | §1.1, §3.1, §3.3, T4.2 |

- **Round 4**: pending — two fresh independent seats on v3, verifying this
