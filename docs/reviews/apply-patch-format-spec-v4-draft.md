...
Evidence: `~/.codex/sessions/2026/09/{13,14,15}` rollouts · v0 archive: `docs/reviews/apply-patch-format-spec-v0-draft.md` · v1 archive: `docs/reviews/apply-patch-format-spec-v1-draft.md` · v2 archive: `docs/reviews/apply-patch-format-spec-v2-draft.md` · v3 archive: `docs/reviews/apply-patch-format-spec-v3-draft.md`

Status: SPEC — v4 (post round-4 review; resolution maps in §7). No code changes
until a fresh review round (round 5) returns zero Blocking and zero Major.
...
lines`, `Failed to read file`, `Failed to write file` — the qwen row's 30
VERIFY = 27 find-expected + 2 read-file + 1 write-file; the sole write-file
failure is `call_be0ac666…`, 09-14 08:22:44Z); MISSING = F3; UNSUPPORTED = pre-seam
...
the `+`-prefixed terminator). Most recent day (09-15) alone: 13 of 21 glm
...
parse-rejection rate is ~70% (as of the pinned snapshot), an order of
magnitude above qwen's ~1%.
...
2. Behavioral: mocked model emits `function_call apply_patch` with an
   ...
   - `ev_apply_patch_function_call(call_id, patch)` in
     ...
     and :1030-1035; generic constructor at :933-942);
...
| D-M2 T4.2 scaffold | New helper pair named: `ev_apply_patch_function_call` (responses.rs, pattern :933-942/:1030-1038 — v3: corrected to :1025-1028/:1030-1035, verified exact in v4) + `mount_apply_patch_function_call` (apply_patch_cli.rs); `mount_apply_patch` documented as unusable for the function path (custom_tool_call vs `matches_kind` Function-only, registry.rs:548-556) | §4 T4.2/2b |
...
|---|---|---|
| E-N1 / F-F1 qwen row | Snapshot pinned (2026-09-15T23:30:11Z, 83/162/86 rollout files); qwen row re-tabulated under the strict definitions (30 VERIFY / 2 OTHER — the two `multiple operations target …` rejections named with call_ids and timestamps); "reproduces exactly" reworded to "at the pinned instant", with the independent reproductions named; freshness note added (round-3 snapshot copy: qwen 439, 6 PARSE ≈ 1.4%); glm row declared snapshot-stable (v4: restated — two explicit pins, qwen 2026-09-15T23:30:11.662Z / glm 2026-09-15T23:34:04.678Z; the file triple 83/162/86 belongs to the snapshot copy, the pin-instant triple is 83/162/72; stability qualified to the snapshot copy — see the Round-4 map, G2 row) | TL;DR, §1.2 |
...
- **Round 5**: pending — two fresh independent seats on v4, verifying this
