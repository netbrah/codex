...
Companion SoT: `docs/responses-compat-seam.md`.

Status: TASK BREAKDOWN — drafted (stage 3). No implementation until this
breakdown passes a fresh 2-seat cross-review (stage 4) with zero Blocking and
zero Major.
...
- Line references below are verified at HEAD as of the v5 spec (round-4/5
  seats' line-ref sweeps: 0 wrong). They may drift as earlier items land;
  each item's first step is to re-verify its own references before editing.
...
| 4 | `test_apply_patch_cli_rejects_invalid_hunk_header` (`codex-rs/core/tests/suite/tool.rs`:386) | :393 | exact-stderr updated to the new StartedPatch message (CLI path; spec T1.12) | Item 3 |
...
`streaming_parser.rs`:819 / :784 / :797, and the 24 golden scenario fixtures
...
| 2 | T2 (T2.1-4) | P1: full format taught in the function-tool `patch` argument description (non-OpenAI path) + drift guard + self-consistency + snapshot update | `codex-rs/core/src/tools/handlers/apply_patch_spec.rs`, `apply_patch_spec_tests.rs` (+ snapshot) | 0/13 (new T2.1 test is red first) |
| 3 | T3 + T1.4, T1.5, T1.12, T1.14 (P3.1-P3.4) | Teachable parse-error messages: StartedPatch append, DeleteFile replacement, handler absent/non-string `patch`, boundary pre-pass | `streaming_parser.rs` (:193, :222), `parser.rs` (:268/:271), `codex-rs/core/src/tools/handlers/apply_patch.rs` (:534-541), `apply_patch_tests.rs`, `tests/suite/tool.rs` (:393), parser tests | 12/13 |
...
5. New sub-cases, each: write test → run (RED if new behavior, green lock
   ...
   - T1.13 golden scope: the 24 scenario fixtures under
...
6. New P2 tests live in a new sibling test file per repo convention
   (`#[cfg(test)] #[path = "streaming_parser_p2_tests.rs"] mod tests;` —
   name may follow the file's existing pattern if a better one exists;
   the rewritten assertion :838 stays in the existing inline module).
...
5. T2.3: the existing full-snapshot test
   `create_apply_patch_function_tool_matches_expected_spec`
   (`apply_patch_spec_tests.rs`:40) now fails on the new text → update the
   snapshot (`just test -p codex-core apply_patch` → `cargo insta
   pending-snapshots` → review → `cargo insta accept -p codex-core` scoped
   to this test's snapshot) → green. Intentional change, not a regression —
...
`just fmt`.
**DoD:** T2.1-4 green; snapshot updated and reviewed; no change to the
...
- **P3.3** handler, `apply_patch.rs`:508-560 — the collapsed
...
5. Land P3.3 (split handler messages) → new T3.1 tests (see below) green.
6. Land P3.4 (both boundary strings) → ledger #5-13 green.
   After each sub-step: run the item gates; never leave the tree red at a
   sub-step boundary that ends with a gate run (the red window is
   assertion-rewrite → site implementation within one item, as designed).
7. T3.1 new tests (`apply_patch_tests.rs`): `patch` absent → exact
   §3.3.3a message; `patch` present but non-string (e.g. an object) →
   exact §3.3.3b message.
8. T3.2 new test: handler-level F1-shaped raw Add-File patch executes
...
9. T1.14 is satisfied by steps 2+6 (the parser.rs rewrites are the T1.14
...
1. T4.1: new test in `apply_patch_cli.rs` — the default `test_codex()`
   ...
   `.single_request()`/`.requests()`, `responses.rs`:39-58) carries
...
2. New helper `ev_apply_patch_function_call(call_id, patch)` in
   ...
   constructor `ev_function_call` :933-943):
...
3. New `mount_apply_patch_function_call` in `apply_patch_cli.rs`,
   ...
   rejects the kind mismatch (`registry.rs`:548-556, message :550).
...
2. Spec final pass (`docs/responses-compat-apply-patch-format.md`): status
   line → implemented (record the five item commit SHAs); leave the §7
   review log and all archives untouched.
...
- No existence check on any path (Add-File overwrites; Delete-File never
  reads); no Update-File leniency; no `strict:true`; no client-side retry
  of `{}` calls; no per-model parser modes.
...
1. Item 1's new test module: sibling `*_tests.rs` file (repo convention
   for NEW modules) — confirm the exact module name/`#[path]` against the
   file's existing pattern before coding.
2. Item 3 step 3: does the extended P3.1 message keep the substring
...
3. Item 4 step 3: `apply_patch_responses` function-call variant vs a
...
4. Item 6 sub-step 2: the new binary name (`codex-aplfix1`) — confirm the
