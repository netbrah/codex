...
Evidence: `~/.codex/sessions/2026/09/{13,14,15}` rollouts · v0 archive: `docs/reviews/apply-patch-format-spec-v0-draft.md` · v1 archive: `docs/reviews/apply-patch-format-spec-v1-draft.md` · v2 archive: `docs/reviews/apply-patch-format-spec-v2-draft.md` · v3 archive: `docs/reviews/apply-patch-format-spec-v3-draft.md` · v4 archive: `docs/reviews/apply-patch-format-spec-v4-draft.md`
...
(each row as of its own pinned instant in §1.2 — the session
directory is live; the round-3 freshness snapshot gives qwen 439 calls,
6 PARSE ≈ 1.4%.)
...
The `invalid patch: ` prefix is `ParseError::InvalidPatchError`'s Display
format (`parser.rs:57`); the parser-level strings (without the prefix) are
what P3.4 edits. (v1 mis-bucketed these seven glm rejections as "VERIFY";
they are parse-path rejections — §1.2 recount history.)

Origin: the `check_start_and_end_lines_strict` pre-pass
(`codex-rs/apply-patch/src/parser.rs:256-274`, messages at :268/:271), run
...
(re-verified byte-exact by rounds 2 and 3):
`~/.codex/sessions/2026/09/14/rollout-2026-09-14T03-25-55-*` — patch is
just `*** Add File: …` + `*** End Patch` (no Begin marker, no content);
`…/rollout-2026-09-14T07-58-02-*` — patch ends with a
...
Method: every rollout under `~/.codex/sessions/2026/09/{13,14,15}/` (on-disk layout is `YYYY/MM/DD`, v4);
unique `call_id` for `function_call name=apply_patch`; model read from
each session's first `turn_context` payload; outcome from the matching
...
lines`, `Failed to read file`, `Failed to write file`, `Failed to find context`
(file_update.rs:110) — the qwen row's 30 VERIFY = 27 find-expected +
2 read-file + 1 write-file; the sole write-file failure is
`call_be0ac666…`, 09-14 08:22:44Z); MISSING = F3; UNSUPPORTED = pre-seam
build; OTHER = other rejections. **Snapshot (v4):** the session directory
is live (rollout lines are still being appended), so each row is pinned to
its own last included event — the qwen row at **2026-09-15T23:30:11.662Z**,
the instant the qwen cumulative call count first reached 413 (83/162/72
rollout files for 09-13/09-14/09-15 at that instant); the glm row at
**2026-09-15T23:34:04.678Z** (19:34:04 EDT), its last glm event
(`call_d6cac804…`, OK — the row's final two PARSE events, `call_1bcbc827…`
23:30:46.881Z and `call_1c41b322…` 23:31:03.433Z, fall after the qwen pin).

| Model | unique calls | PARSE | OK | VERIFY | other |
|---|---|---|---|---|---|
| glm-5.2 | 37 | **26 (~70%)** | 11 | 0 | — |
| qwen3.8-27b | 413 | **5 (~1%)** | 374 | 30 | MISSING 1, UNSUPPORTED 1, OTHER 2 |

glm PARSE breakdown (26; v3-corrected — the v2 "13 F1-class" miscounted,
and the v2 list summed to 22, not 26): 17 F1-class (raw Add-File content:
10 `#`-first-line, 3 `VERDICT:`-first-line, 3 `@@`-first-line, 1
`---`-first-line), 2 F2-class (raw Update content), 5 missing
`*** Begin Patch` (F4), 2 missing `*** End Patch` (F4; both patches end with
the `+`-prefixed terminator — final line `+*** End Patch`, never bare;
v5: corrected from "one", both patches re-derived byte-exact). Most recent
day (09-15) alone: 13 of 21 glm
calls rejected (~62%). Zero glm call in the three days failed with a
filesystem verification error.

Recount history (disclosed in full; v3):
...
- Round-2 seat D strict recount (canonical for the glm row): glm 26/37 —
  reproduced exactly, event-by-event, by round-3 seats E and F and v4
  (glm row pin restated: 2026-09-15T23:34:04.678Z); the glm row is
  snapshot-stable — no glm data after its row pin (09-15 19:34:04 EDT) as
  of the round-3 freshness snapshot (next glm event: 2026-09-16T00:39:58Z).
- Round-3 seats E/F (qwen row): at the pinned instant the qwen row
  reproduces exactly — 413 = 374 + 5 + 30 + 1 + 1 + 2. The v2 table's
  31 VERIFY / 1 OTHER was a coarser bucketing: the two `invalid patch:
  multiple operations target …` rejections (`call_30690924…`
  2026-09-15T20:20:08Z, `call_096536c4…` 21:25:21Z) are OTHER under the
  method's own definitions. The exact reproduction at the pinned instant is
  what validates the recount pipeline (the same pipeline produced v1's glm
  mis-bucket — hence the pin is load-bearing).
- Freshness (v4): the round-3 freshness snapshot — a copy of the session
  directory taken in the window 2026-09-16T00:23:58.623Z (the 439th qwen
  call, `call_ade5c987bda…`) ≤ t < 2026-09-16T00:27:48.898Z (the 440th) —
  gives 83/162/86 rollout files and qwen 439 calls with 6 PARSE (≈1.4%);
  the ~1% rate holds and the glm row is unchanged at that copy (next glm
  event: 2026-09-16T00:39:58Z). (v3's "00:22Z" label was nominal — the
  window is the reproducible statement.) Post-snapshot live data, not part
  of either row: four further glm PARSE calls through 09-15 21:44:55 EDT —
  `call_95179bbe…` 00:39:58Z, `call_35381ef9…` 00:40:02Z, `call_2e757768…`
  01:40:35Z, `call_0d06d3f7…` 01:44:55Z (live glm row 30/41 ≈ 73% as of the
  v4 snapshot).

Conclusion: unchanged in direction, worse in magnitude — glm-5.2's
parse-rejection rate is ~70% (as of the pinned snapshot), ≈58× qwen's
~1% (26/37 = 70.27% vs 5/413 = 1.21%).
...
- The FORMAT block is the **canonical** form the model must emit; both the
  ...
  prefixed line (streaming_parser.rs:307-349; probed on the unmodified
...
- Length (v2, measured): tool description 126 chars + `patch` argument
  description 2,198 chars = 2,324 chars / 401 words (v3: 380 was the
  argument-description-only count) ≈ 550–615 tokens by
...
text). Full new message strings shown. Prefix status (v3): sites 1 and 4
extend the pre-existing sentence (old text remains an exact prefix
substring); site 2 is a wholesale replacement; site 3 re-quotes the
argument name (backticks → single quotes) and splits the one collapsed
error into two.
...
4. **(v2, round-2 seat D M1 recommendation) Boundary pre-pass messages** —
   `check_start_and_end_lines_strict` (parser.rs:256-274), the single owner
...
(parser.rs:558 — 7 exact-string assertions, v3-corrected: 6 Begin-message
...
Total red assertions: 13 (3 streaming + 1 CLI + 2 + 7). A repo-wide search
...
2. Behavioral: mocked model emits `function_call apply_patch` with an
   ...
   - `ev_apply_patch_function_call(call_id, patch)` in
     ...
     `ev_apply_patch_exec_command_call_via_heredoc` (responses.rs:1025-1028
     and :1030-1035; generic constructor at :933-943);
...
- §3: add P1/P2/P3 as the format-remediation layer of the function-tool
  capability, with the 2026-09-13..15 glm-5.2 evidence (26/37 ≈ 70% as of
  the §1.2 pinned snapshot; 09-15 alone 13/21 ≈ 62%) vs qwen 5/413 ≈ 1%.
...
|---|---|---|
| D-M1 glm table | Re-tabulated per the strict documented method: glm 26/37 ≈ 70% PARSE (19 content-class + 7 boundary-class F4), OK 11, VERIFY 0; full recount history disclosed (v0 27/276 retracted; seat A 18/36; v1 19/36 mis-bucketed; denominator 36→37); qwen 413 exact reproduction named as method validation. **(v3: pinned-snapshot refinement, 30/2 strict VERIFY/OTHER split, F1-class 17 — see R3 map.)** | TL;DR, §1.2 |
| D-M1 (class) F4 | Named in §1.1 with exact messages, pre-pass origin (parser.rs:256-274, run at :193-199 before the streaming parser; v3: range corrected), rollout examples, and per-day subset | §1.1 F4 |
...
| D-M2 T4.2 scaffold | New helper pair named: `ev_apply_patch_function_call` (responses.rs, pattern :933-943/:1030-1038 — v3: corrected to :1025-1028/:1030-1035, verified exact in v4; v5: constructor range end :942→:943) + `mount_apply_patch_function_call` (apply_patch_cli.rs); `mount_apply_patch` documented as unusable for the function path (custom_tool_call vs `matches_kind` Function-only, registry.rs:548-556) | §4 T4.2/2b |
| C-N1 / D-m1 false `@@` note | Replaced with the verified statement: parser auto-creates the first chunk from a prefixed line, no `@@` required (streaming_parser.rs:307-349, probed, upstream-pinned at parser.rs:402; v3: range corrected; v4: tail :347 → :349); real deltas the other way (zero-chunk hunk rejected, second `@@` on empty chunk rejected, new `@@` after `*** End of File` accepted) | §3.1 notes |
...
| C-N4 / D-m3 length | Re-measured on the final v2 text: 2,324 chars / 401 words (v3: 380 was arg-only) ≈ 550–615 tokens; 1K gate noted | §3.1 notes |
...
- **Round 3** (2026-09-15/16): two fresh independent seats on v2, both
  **CHANGES-REQUESTED, 0 Blocking**. Every finding is in the §1.1/§1.2
  evidence narrative and in counts/refs; the design (P1/P2/P3/P3.4) was
  judged implementable as written by both seats, every parser claim
  re-probed PASS on the unmodified binary, and the glm row reproduced
  exactly by both:
  - Seat E (`docs/reviews/apply-patch-format-spec-r3-seatE.md`): 2 Major
    (N1: the qwen "reproduces exactly" claim is not reproducible — the
    session dir is live; at the 413 instant the strict split is 30 VERIFY /
    2 OTHER, not 31/1; no snapshot pinned; N2: the glm PARSE breakdown
    sums to 22, not 26 — F1-class is 17, not 13), 3 Minor (N3: session-path
    notation wrong in §1.1/§1.2 vs the actual `2026/09/{13,14,15}/`
    layout; N4: the "exact prefix substring" claim is false for 2 of the 4
    P3 sites; N5: `test_parse_patch_lenient` red count is 7, not 5), 2 Nit
    (N6: 380 → 401 words; N7: three line ranges drifted from the current
    tree).
  - Seat F (`docs/reviews/apply-patch-format-spec-r3-seatF.md`): 2 Major
    (F1: qwen 413 = first-413-by-timestamp prefix, cutoff
    2026-09-15T23:30:11Z; fresh recount 439 = 6 PARSE ≈ 1.4%; F2: 13 → 17
    F1-class, all 17 call_ids listed), 3 Minor (F3: the §1.1 F4 quotes omit
    the `invalid patch: ` Display prefix; F4: red count 7 not 5; F5: prefix
    claim false for P3.2/P3.3), 2 Nit (F6: 401 words; F7: §6's
    26/37-vs-09-15 date conflation). F independently corroborated E's
    N1/N2/N4/N5/N6 (and vice versa); N3/N7 and F3/F7 are seat-specific.

### Round-3 resolution map (v2 → v3)

| Finding | Resolution | v3 location |
|---|---|---|
| E-N1 / F-F1 qwen row | Snapshot pinned (v5: two explicit pins — qwen 2026-09-15T23:30:11.662Z with pin-instant file triple 83/162/72, glm 2026-09-15T23:34:04.678Z; v3 had paired the qwen pin with the 83/162/86 freshness-copy triple — Round-4 map, G2 row); qwen row re-tabulated under the strict definitions (30 VERIFY / 2 OTHER — the two `multiple operations target …` rejections named with call_ids and timestamps); "reproduces exactly" reworded to "at the pinned instant", with the independent reproductions named; freshness note added (round-3 snapshot copy: qwen 439, 6 PARSE ≈ 1.4%); glm row declared snapshot-stable (stability qualified to the round-3 freshness-snapshot copy; Round-4 map, G2 row) | TL;DR, §1.2 |
| E-N2 / F-F2 glm breakdown | Corrected: 17 F1-class (10 `#` + 3 `VERDICT:` + 3 `@@` + 1 `---`) + 2 F2 + 7 F4 = 26; the v2 sum-22 error named | §1.2 |
| E-N3 | Session-path notation: §1.1/header fixed to the actual `~/.codex/sessions/2026/09/{13,14,15}/` layout in v3 (the v2 header was already correct; its v1→v2 fix is noted here); §1.2's Method line still carried the non-existent dash form in v3 (v2's was `2026-09-{13,14,15}`) and was corrected in v4 — the on-disk layout is `YYYY/MM/DD` | §1.1, §1.2 |
| E-N4 / F-F5 | §3.3 intro reworded: sites 1 and 4 preserve the prefix; site 2 is a replacement; site 3 re-quotes the argument name and splits the collapsed error | §3.3 |
| E-N5 / F-F4 | `test_parse_patch_lenient` red count corrected to 7 (6 Begin + 1 End, enumerated); total red assertions 13 | §3.3, T1.14 |
| F-F3 | `invalid patch: ` Display prefix added to both §1.1 F4 quotes; parser-level vs harness-level strings distinguished; and §1.1's re-verification credit extended to round 3 (byte-exact, all 7 F4 rollouts — v4: itemized) | §1.1 F4 |
| E-N6 / F-F6 | Word count 380 → 401 (380 was the argument-description-only count) | §3.1 notes |
| F-F7 | §6 date conflation fixed: 26/37 is the 2026-09-13..15 aggregate (pinned snapshot); 09-15 alone 13/21 ≈ 62% | §6 |
| E-N7 | Line ranges corrected to the current tree (auto-chunk streaming_parser.rs:307-349 (v4: tail); `check_start_and_end_lines_strict` parser.rs:256-274, messages at :268/:271; responses.rs pattern cited by name + line); ranges re-drift under P3's own edits, so code cites symbols where possible | §1.1, §3.1, §3.3, T4.2 |

- **Round 4**: two fresh independent seats on v3 — G (diff/map audit +
  pinned-snapshot recount + consistency) and H (red-state completeness +
  implementability + line-ref sweep + v3-regression hunt).
  - Seat G (`docs/reviews/apply-patch-format-spec-r4-seatG.md`):
    CHANGES-REQUESTED — 0 Blocking, 1 Major, 2 Minor, 2 Nit. G1 (Major):
    §1.2's Method line cites the non-existent `2026-09/{13,14,15}/` layout
    (on disk: `2026/09/{13,14,15}`), so the E-N3 "unified to the actual
    layout" claim failed for §1.2 and the R3 map recorded the wrong form as
    actual. G2 (Minor): the pin sentence conflates instants — the 83/162/86
    file triple is the 00:22Z freshness snapshot's count (pin-instant
    triple: 83/162/72), the glm row's own last event postdates the stated
    pin, and the "no glm data after 09-15 19:31 EDT" stability claim was
    unqualified. G3 (Minor): the 30th qwen VERIFY is a post-apply `Failed
    to write file` (`call_be0ac666…`) absent from the definition's exemplar
    list. G4/G5 (Nit): auto-chunk range ends at :349, not :347; hunk 5's
    wording change not itemized in the R3 map. G independently reproduced
    both rows event-by-event (qwen 413 at 23:30:11.662Z; glm 26/37 with
    the 17+2+5+2 breakdown), re-probed the T2.2 example byte-exact, and
    verified every mandated line ref.
  - Seat H (`docs/reviews/apply-patch-format-spec-r4-seatH.md`):
    APPROVED — 0 Blocking, 0 Major, 1 Minor, 1 Nit. H1 (Minor): the same
    E-N3 defect as G1 (§1.2 Method line + the R3 map's "actual layout"
    citation). H2 (Nit): the R2-map D-M2 row retains the stale
    `responses.rs:1030-1038` range. H verified all 13 red assertions
    line-by-line, all 31 distinct `file.rs:NNN` citations at HEAD (0
    wrong), 8 of 9 round-3 fixes LANDED (E-N3 PARTIAL), T1–T5 executable
    as written with 3/3 mandated probes PASS, and 13/13 v2→v3 hunks trace
    to the R3 map.

### Round-4 resolution map (v3 → v4)

| Finding | Resolution | v4 location |
|---|---|---|
| G1 / H1 session path (Major/Minor) | §1.2 Method line corrected to the actual on-disk layout `~/.codex/sessions/2026/09/{13,14,15}/` (`YYYY/MM/DD`, verified on disk; the dash form never existed); the R3-map E-N3 row and the round-3 seat-E summary restated to record the slash form as actual and that the §1.2 correction landed in v4 | §1.2 Method, §7 (E-N3 row, round-3 summary) |
| G2 pin conflation (Minor) | Two explicit pins: qwen row 2026-09-15T23:30:11.662Z (413th call; file triple 83/162/72 at that instant) and glm row 2026-09-15T23:34:04.678Z (its own last event, `call_d6cac804…` OK); the 83/162/86 triple moved to the Freshness bullet as that snapshot's count (reproducible window: 00:23:58.623Z ≤ t < 00:27:48.898Z); the stability sentence qualified to the freshness-snapshot copy (next glm event 2026-09-16T00:39:58Z); the R3-map E-N1 row annotated. (v4 note: the round-4 report's suggested 23:31:03.433Z glm pin was itself one event short — at that instant the row is 26 PARSE + 10 OK = 36; the 11th OK is `call_d6cac804…` @ 23:34:04.678Z, re-derived event-by-event in v4.) | §1.2 Snapshot, seat-D bullet, Freshness, TL;DR, §7 E-N1 row |
| G3 30th VERIFY (Minor) | VERIFY definition exemplars extended with `Failed to write file`; the qwen row's 30 VERIFY decomposed 27 find-expected + 2 read-file + 1 write-file, the write-file call named | §1.2 definition |
| G4 auto-chunk tail (Nit) | `streaming_parser.rs:307-347` → `307-349` in all three cites (branch span verified: :307-316 / :318-327 / :329-338 / :340-349) | §3.1 note, R2-map C-N1/D-m1 row, R3-map E-N7 row |
| G5 unitemized wording (Nit) | F-F3 row extended to cover §1.1's re-verification credit (byte-exact, all 7 F4 rollouts) | R3 map F-F3 row |
| H2 stale R2-map range (Nit) | D-M2 row annotated: the v2 range `:1030-1038` corrected in v3 to `:1025-1028/:1030-1035`, verified exact in v4 | R2 map D-M2 row |

- **Round 5**: two fresh independent seats on v4 — I (R4-map landing
  verification + full v3→v4 diff audit + event-by-event re-derivation of all
  14 load-bearing pinned claims from a fresh snapshot + arithmetic/
  consistency sweep) and J (R4-fix verification + red-state completeness +
  T1–T5 implementability with probes + line-ref sweep + v4-new-claims audit
  + regression hunt). **Full round: 0 Blocking + 0 Major — the spec review
  loop's termination condition is met.**
  - Seat I (`docs/reviews/apply-patch-format-spec-r5-seatI.md`): APPROVED —
    0B/0M/1m/3n. All six R4-map rows landed at their cited lines; the G2
    special check holds (glm row is 36 = 26 PARSE + 10 OK at 23:31:03.433Z
    and 37 = 26 + 11 at 23:34:04.678Z; 11th OK = `call_d6cac804…`); 9/9
    v3→v4 hunks trace to the R4 map or bookkeeping; P1 code block, §3.2,
    §3.4 byte-identical v3→v4; all pinned claims reproduce exactly (fresh
    snapshot 2026-09-16T02:25:38Z), including the 17/17 F1 quoted-line
    cross-checks and the 83/162/86 + 439/6 freshness window. Findings:
    I-M1 (the two missing-End patches BOTH end `+*** End Patch`, not one),
    I-N1 ("an order of magnitude above" ≈ 58×), I-N2 (R3-map E-N1 row
    opening still pairs the qwen pin with the 83/162/86 triple), I-N3
    (VERIFY exemplar list omits `Failed to find context`).
  - Seat J (`docs/reviews/apply-patch-format-spec-r5-seatJ.md`): APPROVED —
    0B/0M/1m/1n. All R4 rows re-verified (layout on disk, :349 tail,
    responses.rs ranges); 13/13 red assertions located line-by-line; T1–T5
    executable as written, 3/3 debug-binary probes PASS (T2.2 byte-exact,
    F1 raw Add-File still rejected, context anchoring exact); 32 line
    citations, 0 wrong; v4 new claims re-derived exact from a fresh
    snapshot (439/6 boundary, freshness window, four post-snapshot glm
    calls, 30/41 ≈ 73% live); 9/9 v3→v4 hunks trace. Findings: J-M1 (VERIFY
    exemplar list omits `Failed to find context`, file_update.rs:110 —
    corroborates I-N3), J-N1 (`ev_function_call` closes at
    responses.rs:943, cited as :942).

  Round-5 findings are minor/nit only; all five are fixed in v5 (post-loop
  cosmetic cleanup — none affects the design, pins, counts, red state, P1
  text, or the T-plan). Each is source-verified in the Round-5 resolution
  map below, and all five are Mandate 0 of the task-breakdown
  cross-review (stage 3), which re-verifies them at source before any code
  change.

### Round-5 resolution map (v4 → v5)

| Finding | Resolution | v5 location |
|---|---|---|
| I-M1 (Minor) | "one patch ends with the `+`-prefixed terminator" → both patches do (final line `+*** End Patch`, never bare) — both patches' final lines re-derived byte-exact from the rollouts (`call_10f109a3…` 09-14, `call_8ee1b0c1…` 09-15) | §1.2 glm breakdown |
| I-N3 / J-M1 (corroborating, Minor) | VERIFY exemplar list gains `Failed to find context` (file_update.rs:110); the two live calls (`call_a9cf9f97…`, `call_69f201be…`, both 09-15 23:56Z) sit after the qwen pin and inside the freshness snapshot's 439 — outside every pinned row, so no pinned claim changes | §1.2 definition |
| J-N1 (Nit) | `responses.rs:933-942` → `:933-943` (fn spans :933–:943; verified at HEAD) in both cites | T4.2, R2-map D-M2 row |
| I-N1 (Nit) | Conclusion "an order of magnitude above" → "≈60× qwen's ~1%" (70.27% / 1.21% ≈ 58×) | §1.2 Conclusion |
| I-N2 (Nit) | R3-map E-N1 row opening restated — v3's superseded pin/triple pairing moved out of the opening, mid-row annotation deduplicated | R3 map E-N1 row |

- **Spec review stage: COMPLETE** — round 5 (two fresh independent seats)
  returned zero Blocking and zero Major. v5 carries the five minor/nit
  fixes above, all source-verified. Next: stage 3 (task breakdown); its
  2-seat cross-review re-verifies the five v5 fixes as Mandate 0. No code
  changes until that cross-review returns zero Blocking and zero Major.
